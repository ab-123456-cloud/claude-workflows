export const meta = {
  ...
  whenToUse: 'When you want deep research but on a hard token budget — e.g. constrained usage window, or running under no supervision. Pass the question as args (a string), or { question, capTokens, bdata } where capTokens is a TOTAL-token budget (default 1.2M; a focused run lands ~0.9-1M) and bdata:true opts agents into the Bright Data CLI — set it ONLY after the orchestrator verified the CLI is installed (probe once, e.g. `command -v bdata`); the default assumes it is absent. Pass args as a real JSON object, not a stringified one.',
  phases: [
    { title: 'Scope', detail: 'decompose into <=4 angles' },
    { title: 'Search', detail: 'Haiku, one per angle, WebSearch (+ bdata only if enabled via args)' },
    ...
    { title: 'Synthesize', detail: 'Sonnet (pinned) — cheap, never the frontier model' },
  ],
}
...
const REFUTATIONS_REQUIRED = 2          // kill = BOTH voters refute; but <2 valid votes ALSO kills (agent failure ≠ survival — unverified is not confirmed)
...
const useBdata = !!(A && A.bdata)       // orchestrator probes the CLI once and opts in — agents must never burn turns discovering a missing binary
...
const SCRATCH = './.frugal/' + (slug || 'research')
...
const scope = await agent(
  ...
  { label: 'scope', schema: SCOPE_SCHEMA, model: 'haiku', effort: 'low' }
)
...
const SEARCH_TOOLING = useBdata
  ? 'PREFER the Bright Data CLI: run `bdata search "<query>" --json` via Bash FIRST (broader reach, fewer blocks). ' +
    'ALSO run WebSearch and merge both result sets. If `bdata` errors, fall back to WebSearch alone. '
  : 'Use WebSearch. Do NOT probe for optional CLIs (bdata etc.) — assume they are absent. '
...
const FETCH_TOOLING = useBdata
  ? 'PREFER `bdata scrape "<url>" --format markdown` via Bash (Bright Data Web Unlocker — handles blocks/paywalls). If `bdata` errors, use WebFetch instead.'
  : 'Use WebFetch. Do NOT probe for optional CLIs — assume they are absent.'

const FETCH_PROMPT = (s, ang) =>
  '## Source extractor\nQuestion: "' + Q + '"\nURL: ' + s.url + '\nTitle: ' + s.title + '\nVia: ' + ang + '\n\n' +
  '1. Fetch the page: ' + FETCH_TOOLING + '\n' +
...
const VERIFY_PROMPT = (c, v) =>
  ...
  'WebSearch' + (useBdata ? ' (or bdata)' : '') + ' for contradicting evidence. refuted=true if unsupported by the quote / contradicted / ' +
...
const fetched = await pipeline(
  ...
  a => {
    ...
    return agent(SEARCH_PROMPT(a), { label: 'search:' + a.label, phase: 'Search', schema: SEARCH_SCHEMA, model: 'haiku', effort: 'low' })
  ...
  sr => {
    ...
    return parallel(novel.map(s => () => {
      ...
      return agent(FETCH_PROMPT(s, sr.angle), { label: 'fetch:' + host, phase: 'Fetch', schema: EXTRACT_SCHEMA, model: 'haiku', effort: 'low' })
...
// ─── Synthesize — PINNED to Sonnet (never inherits the frontier model; that's the cost guarantee) ───
phase('Synthesize')
